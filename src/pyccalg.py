...
def _linear_program_pulp(num_vertices, edges, graph):
    ...
    x_vars = {i: plp.LpVariable(cat=plp.LpContinuous, lowBound=0, upBound=1, name='x_{0}'.format(i)) for i in
              range(vertex_pairs)}
        ...
        for j in range(i + 1, num_vertices):
            ...
            for k in range(num_vertices):
                if k != i and k != j:
                    ...
                    constraint = plp.LpConstraint(e=expr, sense=plp.LpConstraintLE, rhs=0,
                                                  name='constraint_{0}'.format(c_count))
